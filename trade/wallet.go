...
import (
	...
	contract "awesomeProject/contract"
	...
	case "":
		...
		for i := 1; i < len(p.PuppetWallets); i++ {
			go func(index int) {
				...
				log.Infof("walletAddress: %s amount: %d", p.PuppetWallets[index].From, sendBalance.Int64())
			}(i)
		}
	...
	default:
		...
		for i := 1; i < len(p.PuppetWallets); i++ {
			...
			go func(index int, contract *contract.Erc20Contract) {
				...
				log.Infof("walletAddress: %s amount: %d", p.PuppetWallets[index].From, tokenBalance.Int64())
			}(i, erc20Contract)
